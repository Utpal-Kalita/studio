...
export default function MoodTrackingPage() {
  ...
  useEffect(() => {
    if (user) {
      const fetchHistory = async () => {
        ...
        try {
          const moodEntriesCol = collection(db, 'moodEntries');
          const q = query(
            moodEntriesCol, 
          ...
          const history = querySnapshot.docs.map(doc => {
            ...
            return { 
              id: doc.id, 
              ...data,
              // Firestore Timestamps need to be converted to ISO strings for date manipulation / display
  ...
  const handleSaveMood = async () => {
    ...
    setIsSaving(true);
    const newEntryData = { // Omit id, it will be generated by Firestore
      userId: user.uid,
      mood: selectedMood,
      journal: journalText,
      date: new Date(), // Store as Firestore Timestamp, will be converted on fetch
    };

    try {
      const moodEntriesCol = collection(db, 'moodEntries');
      const docRef = await addDoc(moodEntriesCol, newEntryData);
      
      // Optimistically update UI, converting date to ISO string for consistency
